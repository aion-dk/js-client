...
if (typeof global !== 'undefined' && global.hasOwnProperty('TEST_MODE') && global.TEST_MODE === true) {
  require('seedrandom')
  Math.seedrandom('test mode')

  sjcl.prng.prototype.randomWords = function (nwords, _paranoia) {
 ...
 */
function voteToPoint(encodingType, vote) {
  ...
  while (!found && tries < 256) {
    ...
    try {
      point = pointFromBits(pointBN.toBits())
      found = true
    } catch (err) {
      // increment
      pointBN.addM(1)
 ...
 * or an array of ids)
 */
function pointToVote(pointString) {
  let point = pointFromBits(sjcl.codec.hex.toBits(pointString))
...
function electionCodeToPrivateKey(electionCode) {
  let hash = hashString(electionCode)
  let privateKey = sjcl.bn.fromBits(sjcl.codec.hex.toBits(hash))
  privateKey = privateKey.mod(Curve.r)

  return sjcl.codec.hex.fromBits(privateKey.toBits())
}
...
module.exports = function() {
  return {
    ...
    'electionCodeToPrivateKey': electionCodeToPrivateKey
  }
}
