...
describe('#validateReceipt', () => {
  ...
  context('given valid receipt', () => {
    ...
    it('succeeds', async () => {
      expect(() => verifier.validateReceipt(receipt)).not.to.throw
    });
    ...
    context('given a different signing key', () => {
      ...
      it('throws validation error', async () => {
        expect(() => verifier.validateReceipt(receipt)).to.throw(InvalidReceiptError, 'Board receipt verification failed')
      });
    });
  });
  ...
  context('given invalid receipt', () => {
    ...
    it('returns throws error', async () => {
      expect(() => verifier.validateReceipt(receipt)).to.throw(InvalidReceiptError, "Receipt string is invalid");
    });
    ...
    context('given an item with broken address', () => {
      ...
      it('returns false', async () => {
        expect(() => verifier.validateReceipt(receipt)).to.throw(InvalidReceiptError, 'BoardItem address does not match expected address')
      });
    });
  });
  ...
  context('when a generic error is thrown', () => {
    ...
    it('bubbles up', () => {
      expect(() => verifier.validateReceipt(receipt)).to.throw(SyntaxError, "Error");
    })
  })
});

describe('#validateReceiptTrackingCode', () => {
  let verifier: AVVerifier;
  const config: LatestConfig = latestConfig;
  ...
  context('given valid tracking code', () => {
    ...
    it('succeeds', async () => {
      expect(() => verifier.validateReceiptTrackingCode(receipt, trackingCode)).not.to.throw
    });
    ...
    context('given a mismatching tracking code', () => {
      ...
      it('throws validation error', async () => {
        expect(() => verifier.validateReceiptTrackingCode(receipt, trackingCode)).to.throw(InvalidTrackingCodeError, 'Tracking code does not match the receipt')
      });
    });
  });
});
